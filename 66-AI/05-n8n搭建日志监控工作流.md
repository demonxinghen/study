...
```text
Command: LATEST_LOG=$(ls -t /home/logs/*.log 2>/dev/null | head -1); if [ -f "$LATEST_LOG" ]; then LAST_POS_FILE="/tmp/n8n_log_position"; [ -f "$LAST_POS_FILE" ] && LAST_FILE_LINE=$(cat "$LAST_POS_FILE") || LAST_FILE_LINE=""; LAST_FILE=$(echo "$LAST_FILE_LINE" | cut -d':' -f1); LAST_POS=$(echo "$LAST_FILE_LINE" | cut -d':' -f2); [ "$LATEST_LOG" != "$LAST_FILE" ] && LAST_POS=0; TMP_BODY="/tmp/n8n_log_body.log"; TMP_OUT="/tmp/n8n_log_output.log"; BYTES=$(tail -c +$((LAST_POS + 1)) "$LATEST_LOG" | tee >(perl -ne 'if (/^\d{2}:\d{2}:\d{2}\.\d{3}/) { print $buf, "\n" if $buf; $buf = $_; } else { chomp; $buf .= " " . $_; } END { print $buf, "\n" if $buf }' | head -2000 > "$TMP_BODY") | wc -c); NEW_POS=$((LAST_POS + BYTES)); echo "$LATEST_LOG:$NEW_POS" > "$LAST_POS_FILE"; echo -e "filename:$LATEST_LOG\n$(cat "$TMP_BODY")" > "$TMP_OUT"; echo "Log written to $TMP_OUT"; else echo "No log files found"; fi
